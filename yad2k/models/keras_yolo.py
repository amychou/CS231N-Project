...
def yolo_filter_boxes_adv(boxes, box_confidence, box_class_probs, threshold=.6, target_class=0):
    ...
    box_classes = K.argmax(box_scores, axis=-1)
    box_class_scores = K.max(box_scores, axis=-1)
    box_adv_scores = tf.gather(box_scores, target_class, axis=-1)
    prediction_mask = box_class_scores >= threshold
    ...
    scores = tf.boolean_mask(box_class_scores, prediction_mask)
    classes = tf.boolean_mask(box_classes, prediction_mask)
    target_scores = tf.boolean_mask(box_adv_scores, prediction_mask)
    return boxes, scores, classes, target_scores

def yolo_eval_adv(yolo_outputs,
              image_shape,
              max_boxes=10,
              score_threshold=.6,
              iou_threshold=.5,
    ...
    boxes, scores, classes, target_scores = yolo_filter_boxes_adv(
    ...
    target_scores = K.gather(target_scores, nms_index)
    return boxes, scores, classes, target_scores
